...
public class EventHandlerHtml {
    ...
    @SubscribeEvent
    public static void factionsLoaded(FactionsLoadedEvent event) {
        EventHandlerFaction.getFactions().values().forEach(faction -> generateFile(faction));
    }

    private static void generateFile(Faction faction) {
        ...
        if (start != -1 && end != -1) {
            ...
            List<Grade> grades = Lists.newArrayList(faction.getGrades());
