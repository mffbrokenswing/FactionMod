	...
	public List<String> getTabCompletions(MinecraftServer server, ICommandSender sender, String[] args, BlockPos targetPos) {
		if (sender instanceof EntityPlayer) {
			...
			if (!EventHandlerFaction.hasUserFaction(player.getUniqueID())) {
				...
				if (args.length == 1) {
					return AutoCompleter.complete(args[0], new String[] { "disband", "invite", "open", "leave", "kick", "claim", "unclaim", "sethome", "home", "set-grade", "remove-grade", "promote", "info", "set-relation", "chest" });
